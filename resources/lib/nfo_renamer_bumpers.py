...
def log(msg, level=xbmc.LOGINFO):
    """Log to Kodi log"""
    xbmc.log('[NFO Renamer Bumpers] ' + str(msg), level)
...
def run_renamer(directory, dry_run=False, recursive=False):
    ...
    notify('Starting Bumpers renaming...', 'NFO Renamer')
    
    log("=" * 70)
    log("NFO RENAMER - BUMPERS")
    log("=" * 70)
    log("Processing directory: {}".format(directory))
    ...
    try:
        ...
        if stats:
            ...
            notify('Bumpers renaming complete: {} renamed, {} skipped'.format(
        ...
        notify('Bumpers renaming failed - check log', 'NFO Renamer Error')
...
def main():
    ...
    directory = None
    dry_run = False
    recursive = True  # Bumpers always need recursive since they're organized in subdirectories
    ...
    if not directory:
        log("No directory configured - please set NFOBumpersPath in addon settings", xbmc.LOGERROR)
        return 1
