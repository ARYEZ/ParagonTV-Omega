...
# Kodi v18 and earlier use xbmcvfs.translatePath()
try:
...
class MyPlayer(xbmc.Player):
    ...
    def onPlayBackStarted(self):
        """Handle when playback starts - Kodi 20 compatibility"""
        self.log("onPlayBackStarted called (MyPlayer) - delegating to onAVStarted")
        self.onAVStarted()

    def onAVStarted(self):
        """Called when audio or video playback starts - Kodi 21 compatible"""
        ...
        try:
            ...
            # Check if this is audio-only (no video) - Kodi 21 approach
...
class TVOverlay(xbmcgui.WindowXMLDialog):
    ...
    def returnToLivePosition(self):
        ...
        # Clear preemption flag first
        self.isPreempting = False

        # Calculate elapsed time during preemption
        elapsedTime = time.time() - self.preemptStartTime
        self.log("Elapsed time during preemption: %d seconds" % int(elapsedTime))
        ...
        # Get the channel we're returning to
        channel = self.preemptedChannel
        self.log("Returning to channel %d" % channel)

        # Calculate current live position by advancing from SAVED preempted state
        # Use the actual player position, not channel state which may be stale
        timedif = elapsedTime
        showPos = self.preemptedPlaylistPos  # Playlist position when preempted
        showTime = self.preemptedPosition  # Actual player time when preempted

        self.log(
            "Starting from position %d at time %d, advancing %d seconds"
            % (showPos, int(showTime), int(timedif))
        )

        # Advance through shows that finished during preemption
        while timedif > 0 and showPos < self.channels[channel - 1].Playlist.size():
            showDuration = self.channels[channel - 1].getItemDuration(showPos)
            timeLeft = showDuration - showTime

            if timedif >= timeLeft:
                # This show finished, move to next
                timedif -= timeLeft
                showPos += 1
                showTime = 0
                self.log(
                    "Advanced to show position %d, remaining time: %d seconds"
                    % (showPos, int(timedif))
                )
            else:
                # Still in the same show, advance the time
                showTime += timedif
                self.log("Advanced to time offset: %d seconds" % int(showTime))
                timedif = 0

        # Wrap around if we went past the end
        if showPos >= self.channels[channel - 1].Playlist.size():
            showPos = showPos % self.channels[channel - 1].Playlist.size()
            self.log("Wrapped around to position: %d" % showPos)

        # Now update the channel state with the calculated live position
        self.channels[channel - 1].setShowPosition(showPos)
        self.channels[channel - 1].setShowTime(showTime)
        self.channels[channel - 1].setAccessTime(time.time())
        self.channels[channel - 1].setPaused(False)

        self.log(
            "Set channel %d to position %d, time %d"
            % (channel, showPos, int(showTime))
        )

        # Update current channel
        self.currentChannel = channel

        # Load and play the playlist at the calculated position
        xbmc.PlayList(xbmc.PLAYLIST_MUSIC).clear()
        if not xbmc.PlayList(xbmc.PLAYLIST_MUSIC).load(
        ...
        # Play the calculated position
        self.log("Playing position %d" % showPos)
        self.Player.playselected(showPos)

        # Wait for playback to actually start and be seekable
        # isPlaying() returns true too early, before video is ready
        waitCount = 0
        while not self.Player.isPlaying() and waitCount < 50:
            xbmc.sleep(100)
            waitCount += 1

        # Additional wait for video to be fully loaded and seekable
        # Wait until we have valid duration info
        waitCount = 0
        while self.Player.isPlaying() and waitCount < 50:
            try:
        ...
        self.log("Video ready, total wait: %d ms" % (waitCount * 100))

        # Seek to the calculated time within the show
        if self.Player.isPlaying() and showTime > 0:
            try:
                self.log("Seeking to time: %d seconds" % int(showTime))
                self.Player.seekTime(showTime)
                # Give seek a moment to take effect
                xbmc.sleep(500)
            except Exception as e:
                self.log("Failed to seek to time: %s" % str(e), xbmc.LOGERROR)

        # Show channel label
        self.showChannelLabel(channel)
        ...
        try:
            ...
            for seriesId in uniqueSeriesIds:
                try:
                    ...
                    seriesRequest = urllib.request.Request(seriesUrl)
                    seriesRequest.add_header("X-Api-Key", sonarrApiKey)
                    
                    seriesResponse = urllib.request.urlopen(seriesRequest, timeout=5)
